...
{
	const seed = remix_utils.random_scalar() // to generate a 32-byte (25-word) but reduced seed
	const mnemonicString = mnemonic.mn_encode(seed, mnemonic_wordsetName)
	const keys = remix_utils.create_address(seed)
	//
	return {
		seed: seed,
...
{ // -> {err_str?, seed?, keys?}
	...
	try {
		...
		keys = remix_utils.create_address(seed)
		if (keys === null) {
...
)
{
	...
	if (!remix_utils.valid_hex(view_key) || (isInViewOnlyMode ? false : !remix_utils.valid_hex(spend_key))) {
		return { err_str: "invalid hex formatting" }
	}
	var public_keys;
	try {
		public_keys = remix_utils.decode_address(address)
	} catch (e) {
		return { err_str: "invalid address" }
	}
	var expected_view_pub;
	try {
		expected_view_pub = remix_utils.sec_key_to_pub(view_key)
	} catch (e) {
	...
	if (spend_key.length === 64) {
		try {
			expected_spend_pub = remix_utils.sec_key_to_pub(spend_key)
		} catch (e) {
	...
	if (typeof seed_orUndefined !== 'undefined' && seed_orUndefined && seed_orUndefined.length != 0) {
		...
		try {
			expected_account = remix_utils.create_address(seed_orUndefined)
		} catch (e) {
